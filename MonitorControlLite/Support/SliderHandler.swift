...
class SliderHandler {
  ...
  func addDisplay(_ display: Display) {
    ...
    let value = display.setupSliderCurrentValue(command: self.command)
    self.setValue(value, displayID: display.identifier)
  }
  ...
  @objc func valueChanged(slider: MCSlider) {
    ...
    for display in self.displays {
      ...
      self.valueChangedOtherDisplay(otherDisplay: display, value: value)
    }
