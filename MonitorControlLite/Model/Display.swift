...
class Display: Equatable {
  ...
  func checkGammaInterference() {
    ...
    guard !self.isDummy, !DisplayManager.shared.gammaInterferenceWarningShown, !self.readPrefAsBool(key: .avoidGamma), !self.isVirtual, !self.smoothBrightnessRunning, self.prefExists(key: .SwBrightness), abs(currentSwBrightness - self.readPrefAsFloat(key: .SwBrightness)) > 0.02 else {
      return
    }
    ...
    if DisplayManager.shared.gammaInterferenceCounter >= 3 {
      ...
      alert.informativeText = NSLocalizedString("An other app seems to change the brightness or colors which causes issues.\n\nTo solve this, you need to quit the other app or disable gamma control for your displays in MonitorControl Lite!", comment: "Shown in the alert dialog")
      ...
      if alert.runModal() != .alertFirstButtonReturn {
        for otherDisplay in DisplayManager.shared.getAllDisplays() {
  ...
  func setupCurrentAndMaxValues(command: Command, firstrun _: Bool = false) {
  ...
  func setupSliderCurrentValue(command: Command) -> Float {
    return self.readPrefAsFloat(for: command)
  }
